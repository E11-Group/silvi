...
function e11_teams($args = array(), $use_page_content = false, $get_field_type = 'get_field', $field_location = '', $prefix = false)
{
    ...
    if ($use_page_content):
        $defaults = array(
            'title'         => $get_field_type($prefix . 'teams__title', $field_location),
            'member'        => $get_field_type($prefix . 'teams_member', $field_location),
            'module_id'     => $get_field_type( $prefix . 'teams_module_id', $field_location),
        );
    else:
        $defaults = array(
            'title'         => false,
