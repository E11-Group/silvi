 ...
 */
function e11_image_boxes($args = array(), $use_page_content = false, $get_field_type = 'get_field', $field_location = '', $prefix = false)
{
    ...
    if ($use_page_content):
        $defaults = array(
            'title'         => $get_field_type($prefix . 'ib_title', $field_location),
            'boxes'         => $get_field_type($prefix . 'ib_box', $field_location),
            'title_style'   => $get_field_type($prefix . 'ib_style', $field_location),
            'module_id'     => $get_field_type( $prefix . 'ib_module_id', $field_location),
        );
    else:
        $defaults = array(
            'title'         => false,
    ...
    if (!empty($data['boxes'])):
        include 'tpl/image_boxes.tpl.php';
    endif;
