...
function e11_scripts(){
	...
	### Core
        ...
        if( ! empty( $api )){
        	wp_enqueue_script('google-map-js', 'https://maps.googleapis.com/maps/api/js?key='. $api, array('jquery'));
        }
...
function e11_tinymce_style() {
    ...
    wp_localize_script('silvi-admin-scripts', 'localized', array('ajaxurl' => admin_url('admin-ajax.php'), 'siteurl' => site_url()));
}
